...
API backend service with RESTful based on Nest.js and TypeORM
...
- RestAPI для управления данными.
...
- [Шифрование](#шифрование)
 ...
 - [decrypt](#decrypt)
 - [hash](#hash)
 - [простая защита методов](#простая-защита-методов)
...
```shell script
git clone https://github.com/fwmakc/api-server
```
...
```shell script
pm2 logs api-server --lines 1000
```
...
- SECURE_METHOD - метод шифрования, по-умолчанию **MD5**,
- SECURE_EXPIRES - срок действия токена шифрования в секундах,
- SECURE_SIMPLE - токен для простой защиты защищенных методов.

> **Миграция**: переменные `HIDDEN_*` переименованы в `SECURE_*` (Phase 3). Декораторы `@Hidden()` → `@Secure()`, `@SimpleHidden()` → `@SimpleSecure()`.
...
> Токены подписываются RS256 (асимметричный ключ). auth-server использует `JWT_PRIVATE_KEY_PATH` для подписи, остальные сервисы проверяют через JWKS (`/.well-known/jwks.json`). Симметричный `JWT_SECRET` больше не используется.
...
- boolean
- created
- date (то же что created, но null по-умолчанию)
- enum
- float (15 знаков, 2 после точки)
- id
- int
- json
- position_asc
...
Они сделаны таким образом, чтобы соответствовать разным требованиям, например переносимости между базами данных.

Базовые типы полей имеют минимальные настройки для работы. Например, вы должны задать название поля в базе данных.
...
Докумкентирование Дочерней сущности
...
> **Безопасность:** При `cascade: true` метод **sanitizeForSave()** автоматически защищает от несанкционированного создания и изменения связанных сущностей через вложенные DTO. См. раздел [Защита полей](#защита-полей).
...
Модель данных в **TypeORM** автоматически формирует базу данных с необходимыми полями и связями. Такие модели представлены файлами с расширением **.entity.ts**.
...
> **Миграция (Phase 2)**: удалены `CommonEntity`, `ProtectedEntity`, `PrivateEntity`, `ClosedEntity`, `ProtectedOneEntity`, `PrivateOneEntity` и их DTO-маркеры. Политика доступа определяется только в контроллерах через `EntityController` с конфигурацией `operations`.

[^ к оглавлению](#оглавление)

## Настройка модели данных

Модель защиты определяется **только в контроллерах** через конфигурацию `operations` в `EntityController`. Сущности и DTO не кодируют политику доступа.
...
| `operations` | Админ | Фильтр | Описание |
|---|---|---|---|
| `read/create/update/delete: 'public'` | не нужен | нет | Открытый доступ (бывший `CommonController`) |
| `read: 'public'`, write: `'owner'` | `allow` байпасит | только запись | Бывший `ProtectedController` |
| all: `'owner'` | `allow` байпасит | чтение + запись | Бывший `PrivateController` |
| read: `'public'`, write: `'admin'` | `if (!isSuperuser) throw` | только админ | Бывший `ClosedController` |

Доступные уровни: `public`, `auth`, `owner`, `admin`, `closed`. `closed` = эндпоинт не регистрируется.

### Пример: owner-модель posts

Чтение открыто, запись только владельцу.
...
Параметры: `name`, `dto`, `entity`, `authTable` (имя связи), `authField` (поле для матчинга), `operations` (уровни доступа).
...
@Controller('posts')
export class PostsController extends EntityController({
  name: 'Посты', dto: PostsDto, entity: PostsEntity,
  authTable: 'users',
...
Каждое поле сущности может иметь уровень доступа для чтения (**read**) и записи (**write**). Доступные уровни:

| Уровень | Чтение | Запись |
|---------|--------|--------|
| `public` | виден всем (по умолчанию) | записывается всеми |
| `auth` | виден авторизованным | записывается авторизованными |
| `owner` | виден владельцу и админу | записывается владельцем и админом |
| `admin` | виден только админу | записывается только админом |
| `closed` | скрыт всегда | запись запрещена |

### @FieldAccess

Декоратор **@FieldAccess** задаёт уровень доступа для конкретного поля:
...
При вызове **create()** и **update()** автоматически вызывается **stripWriteFields()**, которая удаляет из DTO поля, недоступные для записи текущим пользователем. Например, поле с `write: 'admin'` будет удалено из DTO при запросе от не-админа.

> **Поле владельца (bind-field):** поле, совпадающее с `bind.name` (например, `account`), по умолчанию имеет уровень `closed`, если на нём нет `@FieldAccess`. Это предотвращает несанкционированную передачу владения. Чтобы разрешить — нужно явно декорировать: `@FieldAccess({ write: 'admin' })` (только админ) или `@FieldAccess({ write: 'owner' })` (владелец может передать).

### Защита от каскадных операций (sanitizeForSave)

При вызове **create()** и **update()** автоматически вызывается **sanitizeForSave()**, которая обходит дерево связей TypeORM и:

- **Существующие сущности** (с `id`) — проверяется **владение** через `checkOwnership()` (батч-запрос via TypeORM `In()`). Если caller не владеет сущностью — связь удаляется. Если владеет — обрезается до `{ id }`.
- **Новые сущности** (без `id`) — проверяются через **PermissionRegistry**. Если у пользователя нет прав на создание связанной сущности, она удаляется из DTO.
- **Сущности не в реестре** — без `accountTable` считаются глобальными (доступ разрешён). С `accountTable`, но без регистрации — fail-closed (связь удаляется).
- **Админ-байпас** — суперuser (`bind.allow === true`) обходит проверку владения.
- **Auto-assign relation** (поле, совпадающее с `bind.name`) — пропускает проверку владения (это само поле владельца).

Это защищает от IDOR: создания и изменения связанных сущностей через вложенные DTO при `cascade: true`.
...
Мы рекомендуем использовать контроллеры в качестве обертки, а сам функционал реализовывать в сервисах.
...
Однако, могут возникнуть ситуации, когда некоторые методы окажутся для вас избыточными. В таком случае, вы можете просто удалить их из контроллера и контроллера.
...
Важно учитывать, что в сервисах и контроллерах эти методы имеют впереди название сущности и пишутся в стиле camelCase. Например:
...
> **Поиск (`search`)** работает на уровне SQL (`ILIKE`), а не в памяти. `limit`/`offset` применяются **после** фильтрации — пагинация работает корректно.
>
> - **`fields`** — список полей для поиска. Поддерживаются вложенные связи через точку (например, `account.username`).
> - **`terms`** — поисковые слова (регистронезависимые).
> - **`method`** — `"and"` (по умолчанию): все слова должны быть найдены хотя бы в одном из полей; `"or"`: хотя бы одно слово.
...
> Этот метод существует только в частных (**private**) контроллерах.
...
Получить количество записей по условиям
...
Поддерживает те же параметры `where` и `search`, что и `find`. Параметры `limit`/`offset` игнорируются — всегда возвращается общее количество записей, соответствующих фильтрам.
...
Мы предлагаем наследовать базовые сервисные функции и базовый контроллер.

[^ к оглавлению](#оглавление)

### Наследование контроллера

Базовый фабричный метод находится по пути

 /common/entity.controller.ts
...
Уровни доступа задаются через `operations`:
...
@EntityController({
  ...
  entity: PostsEntity,
  operations: {
    read: 'public',    // public | auth | owner | admin | closed
    create: 'owner',
    update: 'owner',
    delete: 'owner',
  },
...
Сервисы имеют условия фильтрации по id учетной записи. Но условия проверки ложатся на контроллеры и внешние вызовы сервисных методов.

[^ к оглавлению](#оглавление)

# Работа с файлами

На текущий момент загрузка файлов возможна только через контроллер. Контроллеры не поддерживаются.
...
## простая защита методов

Если вам нужно быстро защитить некоторые методы от случайного использования, вы можете использовать простой алгоритм проверки.

В контроллере достаточно назначить декоратор

 @SimpleSecure()



В качестве защиты используется простая проверка токена из заголовка Authorization на соответствие строке, заданной в переменной окружения **SECURE_SIMPLE**.

Оба токена обязательно должны быть указаны, иначе метод останется недоступным.

> Данный метод можно использовать только в тестовых целях. В реальных условиях рекомендуется использовать более сложный механизм.

## защита методов

Если вам нужно защитить методы более сложной защитой, вы можете использовать следующий механизм.

Чтобы защитить метод в контроллере, достаточно назначить декоратор

 @Secure()



Ниже описан механизм защиты.

Для защиты используется токен в заголовке Authorization, который строится по простому алгоритму:

- В качестве соли берется значение переменной окружения **SECURE_SECRET**, к нему через точку добавляется текущая временная метка.

- Полученная строка хешируется методом, заданным в переменной окружения **SECURE_METHOD**. По-умолчанию используется метод **MD5**, также поддерживаются **SHA1**, **SHA256**, **SHA512**.

- К хешированной строке через точку добавляется та же временная метка.

Пример кода на js:

```
const timestamp = Date.now();
const string = `${SECURE_SECRET}.${timestamp}`;
const method = `${SECURE_METHOD || 'MD5'}`.toLowerCase();
const hashed = hash(string, method);
const token = `${hashed}.${timestamp}`;
```

Полученный токен проверяется сервером:

- Токен разбивается на две части через точку: хэш и временная метка.

- В качестве соли берется значение переменной окружения **SECURE_SECRET**, к нему через точку добавляется полученная из токена временная метка.

- Полученная строка хешируется методом, заданным в переменной окружения **SECURE_METHOD**.

- Созданный таким образом хэш сравнивается с хэшем, полученным из токена.

Если хэши совпадают, далее проверяется срок годности токена.

Допустимый срок годности токена берется из переменной окружения **SECURE_EXPIRES** как число в секундах.

Из текущей временной метки вычитается временная метка, полученная из токена. Если значение меньше 0, значит токен поддельный. Если значение больше чем допустимый срок годности, значит действие токена истекло.
...
const string = `${SECURE_SECRET}.${tokenTimestamp}`;
const method = `${SECURE_METHOD || 'MD5'}`.toLowerCase();
const hashed = hash(string, method);
...
const timestamp = Date.now();
const expired = timestamp - tokenTimestamp;

if (expired < 0) {
 throw new UnauthorizedException('Token is invalid');
}

if (SECURE_EXPIRES && expired > +SECURE_EXPIRES) {
...
На данный момент контроллеры реализуются в тестовом режиме и могут не работать.
...
Декоратор имеет один аргумент, являющийся типом. Тип по-умолчанию - **undefined** для контроллеров.
...
Защитники используются для ограничения доступа к таким ресурсам как контроллеры.
...
Для них есть все необходимые сервисы, контроллеры.
...
Если сущность имеет отдельную таблицу в базе данных или ей требуется отдельный контроллер/контроллер, то такую сущность лучше вынести в отдельный модуль.
...
Контроллер будет иметь базовое имя:
...
- `db/` — простые CRUD-операции (контроллеры, сервисы, сущности)
...
- `db/` — стандартный CRUD через `EntityController`
