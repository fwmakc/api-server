...
describe('bind — ownership mechanism', () => {
  ...
  it('P13: find with bob bind returns only bob article', async () => {
    const result = await service.find(
      {},
      { id: 2, name: 'account', key: 'id', allow: false },
    );
    expect(result.length).toBe(1);
    expect(+result[0].id).toBe(3);
  });

  it('P10: create with bind sets account relation', async () => {
  ...
  it('P16: create with bind.key=email matches by email', async () => {
    const result = await service.create(
      ...
      [{ name: 'account' }],
      { id: 'alice@test', name: 'account', key: 'email', allow: false } as any,
    );
    expect(result).toBeDefined();
    expect(result.account).toBeDefined();
    expect(result.account.email).toBe('alice@test');
  });
});
